...
export function deviceOut(args, sz, uxn) {
    ...
    if (args[0] === 24) { // 0x18, Console/write
        ...
        } else if (typeof window !== 'undefined' && window.yakuOutput) {
            // 浏览器环境：使用自定义输出函数
            window.yakuOutput(char);
        } else {
            // 回退到 console.log
            console.log(char);
        }
        ...
        if (args[1] !== 0) {
            if (typeof process !== 'undefined') {
                ...
                // 浏览器环境：抛出特殊异常表示正常退出
                throw new Error(`SYSTEM_EXIT_${args[1] & 0x7f}`);
            }
        }
    }
}
