...
export function parseUxntalProgram(programFile, uxn) {
    let programText;
    try {
        programText = readFileSync(programFile, 'utf8');
    } catch (error) {
...
function stripCommentsFSM(programText) {
    ...
    for (let ch_idx = 0; ch_idx < nchars; ch_idx++) {
        const ch = chars.shift();
        const next_ch = chars.length > 0 ? chars[0] : '';
...
function parseToken(tokenStr, uxn) {
    ...
    if (tokenStr.startsWith('?{')) {
        const lambda_count = uxn.lambdaCount;
